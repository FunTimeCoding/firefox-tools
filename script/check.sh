...
for FILE in ${MARKDOWN_FILES}; do
    WORDS=$(hunspell -d "${DICTIONARY}" -p documentation/dictionary/firefox-tools.dic -l "${FILE}" | sort | uniq)
...
for FILE in ${TEX_FILES}; do
    WORDS=$(hunspell -d "${DICTIONARY}" -p documentation/dictionary/firefox-tools.dic -l -t "${FILE}")
